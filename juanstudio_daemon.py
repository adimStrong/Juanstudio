...
- At 6:00 AM: Export data + push to Vercel

Usage:
    python juanstudio_daemon.py          # Run in foreground
    pythonw juanstudio_daemon.py         # Run hidden (no window)

To auto-start on login:
    1. Press Win+R, type: shell:startup
    2. Create shortcut to: pythonw juanstudio_daemon.py
"""
...
def export_data():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            log(f"Export error: {result.stderr[:200]}", "ERROR")
            return False
    except Exception as e:
    ...
    try:
        ...
        subprocess.run(
            [GIT_PATH, "add",
             ...
             "frontend/public/data/analytics-v2.json"],
            cwd=PROJECT_DIR,
            capture_output=True
        )
        ...
        # Commit
        result = subprocess.run(
            [GIT_PATH, "commit", "-m", "Auto-update analytics data"],
            cwd=PROJECT_DIR,
            capture_output=True,
            text=True
        )
...
def daily_push(state):
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "send_daily_report_v2.py", "--project", "juanstudio", "--screenshot-filter", "thismonth"],
            cwd=PROJECT_DIR,
